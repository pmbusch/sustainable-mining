...
#   "Scripts/SI-Figures/Figure2SI_Biodiversity.R"
# Note: desalination cost curves in Figure2_DesalCurves.R
...
# Fig2a1: NZE line only — guide lines + arrows, no desalination, no other scenarios, no contours
p_a1 <- ggplot(filter(data_fig_a, Scenario == "NZE"), aes(Water_Impact, Cost, col = Scenario)) +
  ...
  annotate(
    "text",
    x = x_hc + 200 + 0.015 * diff(xlim_a_full),
    y = y_hc0,
    label = "Higher cost",
    col = "black",
    size = (label_text + 2) * 5 / 14 * 0.8,
  ...
  labs(
    x = expression("Stress-weighted water use 2025-2050 (trillion " ~ m^3 * "-eq)"),
    y = "Total cost 2025-2050 ($T)",
    col = "",
    title = "Cost versus water stress (all materials)"
  ) +
...
# Fig2a2: Add desalination reference points/labels for NZE (Panel A style)
p_a2 <- p_a1 +
  geom_segment(
    data = df_close_a_nze,
    aes(x = Water_Impact, y = Cost, xend = Water_Impact - 300 / desalination_cost / 2, yend = Cost + 300 / 2),
  ...
  geom_point(data = df_close_a_nze, shape = 21, fill = "white", color = "black", size = 2, stroke = 0.4) +
  # fmt: skip
  annotate("text", x = df_close_a_nze$Water_Impact, y = df_close_a_nze$Cost-200,
    label = paste0("Desalination\n", desalination_cost * 100, "¢/m³"),
     color = "black", size = label_text * 5 / 14 * 0.8, hjust = 0.5, angle = -30) +
  geom_segment(
  ...
  geom_point(data = df_close_a2_nze, shape = 21, fill = "#f72f26", color = "black", size = 2, stroke = 0.4) +
  # fmt: skip
  annotate("text", x = df_close_a2_nze$Water_Impact, y =  df_close_a2_nze$Cost-200,
     label = paste0("Desalination\n", desalination_cost2 * 100, "¢/m³"),
     color = "black", size = label_text * 5 / 14 * 0.8, hjust = 0.5, angle = -20)
...
# Fig2a3: Add APS/SPS curves, desal points for remaining scenarios, scenario
# labels, and the least-cost star — NO contour bands / colorbar
p_a3 <- p_a2 +
  geom_line(data = filter(data_fig_a, Scenario != "NZE"), linewidth = 1.1) +
  geom_segment(
  ...
  geom_point(data = df_close_a_other, shape = 21, fill = "white", color = "black", size = 2, stroke = 0.4) +
  geom_segment(
  ...
  geom_point(data = df_close_a2_other, shape = 21, fill = "#f72f26", color = "black", size = 2, stroke = 0.4) +
  geom_text(
    data = filter(data_fig_a, metric == "0%"),
    aes(label = scen_name),
    nudge_y = 60 * c(1.5, 2, 2),
...
scen_colors_cf <- c("NZE" = "#C44E00", "APS" = "#5E8A00", "SPS" = "#1A6FA4")
