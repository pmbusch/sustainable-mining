...
(r <- optInputs |> filter(Parameter == "Discount rate") |> pull(Value))
(mine_life <- optInputs |> filter(str_detect(Parameter, "Mine Life")) |> pull(Value))
(fraction_not_recovered <- optInputs |> filter(str_detect(Parameter, "not recovered")) |> pull(Value))
...
opt_results_demand <- opt_results_demand |>
  filter(ktons_extracted > 0 | capacity_added > 0 | mine_opened > 0) |>
  mutate(
    metric = case_when(
...
data_decomp_demand <- opt_results_demand |>
  ...
  mutate(
    costs = (ktons_extracted *
      ...
      capacity_added * CAPEX_exp_adj / 1e3) *
      share_NiCoCu /
      1e3,
...
slack_costs_demand <- slack_demand |>
  mutate(
    slack_cu_cost = slack_cu * optInputs[optInputs$Parameter == "Slack cost Copper", ]$Value / (1 + r)^(t - 2025),
    slack_ni_cost = slack_ni * optInputs[optInputs$Parameter == "Slack cost Nickel", ]$Value / (1 + r)^(t - 2025),
    slack_co_cost = slack_co * optInputs[optInputs$Parameter == "Slack cost Cobalt", ]$Value / (1 + r)^(t - 2025),
    slack_li_cost = slack_li * optInputs[optInputs$Parameter == "Slack cost Lithium", ]$Value / (1 + r)^(t - 2025)
  ) |>
  group_by(Scenario, metric) |>
  reframe(
